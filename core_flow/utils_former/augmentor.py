...
class FlowAugmentor:
    ...
    def spatial_transform(self, img1, img2, flow, layer, alpha):
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            layer = cv2.resize(layer, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            layer = layer * [scale_x, scale_y]
            alpha = cv2.resize(alpha, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            alpha = alpha[..., None]
        if self.do_flip:
        ...
        layer = layer[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        alpha = alpha[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        return img1, img2, flow, layer, alpha

    def __call__(self, img1, img2, flow, layer, alpha):
        img1, img2 = self.color_transform(img1, img2) # Color Jitter
        img1, img2 = self.eraser_transform(img1, img2) # Eraser Part of Image
        if self.pwc_aug:
            ...
            flow = np.concatenate([flow, np.ones((flow.shape[0], flow.shape[1], 1))], axis=-1) # H, W, 3
            augmented, flow_valid = spatial_augmentor([img1, img2], flow)
            flow = flow_valid[:,:,:2]
            img1 = augmented[0]
            img2 = augmented[1]
        else:
            img1, img2, flow, layer, alpha = self.spatial_transform(img1, img2, flow, layer, alpha)
